...
# Structure to represent a node in the A* expansion
...
class Graph():
    ...
    def primMST(self, d_temp, t):
        ...
        # Make key 0 so tha this vertex is picked as first vertex
        key[0] = 0
        mstSet = [False] * self.V
        sum_weight = 10000
        ...
        for c in range(self.V):
            ...
            # Pick the minimum distance vertex from the set of vertices not yet proessed
            ...
            # update dist value of the adjacent vertices of the pick vertex only if 
            # the current distance is greater than new distance
            # and the vertex in not in the shotest path tree
            for v in range(self.V):
...
# Prim's algorithm is used for MST (greedy approach)
def heuristic(tree, p_id, t, V, graph):
    ...
    if num != 0:
        ...
        d_temp = {}
        key = 0
        # d_temp dictionary stores mapping of original 
        # city no as (key) and new sequential no as value for MST to work
        for i in range(V):
            if i not in visited:
                d_temp[key] = i
                key = key + 1
...
def checkPath(tree, toExpand, V):
    ...
    if tnode.data.c_id == 1:
        ...
        # print("In else")
        depth = tree.depth(tnode) # Check the depth of the tree
        ...
        while tnode.data.c_id != 1:
            ...
            tnode = tree.get_node(str(tnode.datya.parent_id))
        list1.append(0)
...
def startTSP(graph, tree, V):
    ...
    heu = heuristic(tree, -1, 0, V, graph) # Calculate heuristic for node 0 in the tree
    tree.create_node("1", "1", data = TreeNode(0, 1, heu, heu, -1)) # create the 1st node
    fringe_list = {} # Flringe list dictionary FL
    fringe_list[key] = FringeNode(0, heu) # add 1st node in FL
    key = key + 1
    while goalState == 0 :
        minf = sys.maxsize
        # Pick node having min f_value from the fingle list
        for i in fringe_list.keys():
            if fringe_list[i].f_value < minf:
                toExpand.f_value = fringe_list[i].value
                toExpand.c_no = fringe_list
                toExpand.c_id = i
        ...
        if toExpand.c_no == 0 and path == 1:
            ...
            del fringe_list[toExpand.c_id] # Remove the node from FL
            j = 0
            ...
            while j < V:
                if j != toExpand.c_no:
                    ...
                    f_val = val + graph[toExpand.c_no][j] + h # g(parent) + g(parent -> child) + h(child)
